// ─────────────────────────────────────────────
// salary.js — chart + period list rendering
// ─────────────────────────────────────────────
import { fmtVND, fmtVNDFull, fmtShortDate, fmtDate, DAY_VI } from './utils.js';
...
export function buildChart(labels, values) {
    ...
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          x: {
            grid: { display: false },
            ticks: { color: 'rgba(255,255,255,0.18)', font: { size: 9 }, maxTicksLimit: 8, maxRotation: 0 },
            border: { display: false }
          },
          y: {
            grid: { color: 'rgba(255,255,255,0.04)', drawTicks: false },
            ticks: { color: 'rgba(255,255,255,0.18)', font: { size: 9 }, maxTicksLimit: 3, callback: v => fmtVND(v) },
            border: { display: false }
          }
        },
        plugins: {
...
export function renderPeriodList(periods, el) {
  ...
  el.innerHTML = periods.map(p => {
    ...
    return `
      <div class="period-card" style="margin-bottom: 10px;">
